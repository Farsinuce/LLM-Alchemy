...
export async function POST(req: NextRequest) {
  try {
    ...
    // Simple emoji validation function
    const isValidEmoji = (str: string): boolean => {
      if (!str) return false;
      // Check if it's a proper emoji (Unicode ranges for emojis)
      ...
      return emojiRegex.test(str) && str.length <= 2; // Most emojis are 1-2 characters
    };

    // Validate and clean the emoji
    let validatedEmoji = parsedResult.emoji || '✨';
    if (!isValidEmoji(validatedEmoji)) {
      console.log(`[LLM-Alchemy API] Invalid emoji detected: "${validatedEmoji}", using fallback`);
      validatedEmoji = '✨'; // Fallback to sparkles
    }
